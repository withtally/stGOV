...
contract WrappedGovLst is ERC20Permit, Ownable {
  ...
  /// to an initial `transferFrom`. A second transfer of `GovLst` to the fixed alias address on the
  /// `FixedGovLst` will use in the initial wrapped amount rather than the converted amount ensuring that at most 1 extra wei is sent rather than 2.
