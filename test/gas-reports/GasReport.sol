...
// Abstract class for building deterministic, repeatable gas reports. These can be checked in alongside changes to the
// contract code to track the impact of the changes on gas costs for important user actions. Inherit from this class,
...
abstract contract GasReport is Test {
  ...
  // Implement this method to return the name of the report being generated. Used in determining the report filename
  // and put in the report itself as well.
  function REPORT_NAME() public pure virtual returns (string memory);

  // Implement this method if there are certain global state variables you want to populate with non-zero values first
  // before executing scenarios. This is useful in cases where the slot will almost always be non-zero for real users,
  // for example, a global accumulator like `totalSupply` will never be zero for real user, but would be in tests.
  function touchSlots() public virtual;

  // Implement this method with gas scenarios. Call `startScenario`, do setup tasks, call the method you want to track
  // in the report, call `recordScenarioGasReport` _immediately_ after the call you want to track, then call
  // `stopScenario`.
  function runScenarios() public virtual;

  // Call this to start a gas reporting scenario. Provide a unique string name.
  function startScenario(string memory _scenarioName) public inactiveScenario uniqueScenario(_scenarioName) {
  ...
  // User this in the same way you would use Forge's `makeAddr`. It will prepend the name you provide to the active
  ...
  modifier uniqueScenario(string memory _scenarioName) {
    require(!isCompletedScenario[_scenarioName], string.concat("Scenario name is a duplicate: ", _scenarioName));
    _;
  }
  ...
  modifier incompleteScenario() {
    require(
      !isCompletedScenario[currentScenarioName], string.concat("Scenario is already completed: ", currentScenarioName)
    );
    _;
  }
