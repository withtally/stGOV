...
contract MockFullEarningPowerCalculator is IEarningPowerCalculator {
  ...
  function __setEarningPowerAndIsQualifiedForDelegatee(address _delegatee, uint256 _earningPower, bool _isQualified)
    external
  {
    earningPowerOverrides[_delegatee] =
    ...
    returns (uint256 _earningPower, bool _isQualified)
  {
    ...
    if (_storedEarningPower.isOverride) {
      ...
      if (useFixedReturn) {
        return (fixedReturnValue, true);
      }
      uint256 _calculatedAmount = useMultiplier ? (_amountStaked * multiplierBips) / 10_000 : _amountStaked;
